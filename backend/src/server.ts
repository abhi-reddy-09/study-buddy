...
// CORS: per-environment. Production requires ALLOWED_ORIGIN; dev/test allow local Vite ports.
...
app.use(
  cors({
    origin: (origin, callback) => {
      if (!origin || allowedOrigins.includes(origin) || (!isProduction && isDevelopmentOrigin(origin))) {
        callback(null, true);
      } else {
        callback(null, false);
      }
